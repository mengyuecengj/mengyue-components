...
## Brief Introduction
The `Scrollbar` component provides scrolling functionality when content exceeds the container bounds.
You can use `<MYScrollbar></MYScrollbar>` or `<MYScrollbar />` to implement a scrollbar.

## Using Local Scrollbar
...
### Scrollbar Colors
Customize the thumb, track, and hover colors:
