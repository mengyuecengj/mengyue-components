...
## Rounded Corners
Use the `round` prop to add rounded corners to the border. Here we use `borderColor` to set a clear color. For better visualization, we also use `paddingText` to move the text.
...
## Circle Shape
Use the `circle` prop to turn the border into a circle. Here we use `borderColor` to set a clear color. For better visualization, we also use `paddingText` to move the text.
...
Use `colorBg` to set the background color inside the border.
...
<br />
· If `circle` is set, `round` will not take effect. `circle` and `round` can only be used one at a time.
<br />
...
| circle        | Circle shape                         | boolean  | —                                    | `false` |
| colorBg       | Background color                     | string   | Hex or named color                   | —       |
