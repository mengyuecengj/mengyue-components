...
The checkbox component is used for users to select one or more options from multiple choices. mengyue-components has encapsulated this component and conducted vitest testing.
There are two tags for implementing checkboxes:
...
Use the `gap` prop to control the spacing between checkboxes.
...
| checkStrictly | Whether parent-child are strictly linked | boolean | — | `false` |
