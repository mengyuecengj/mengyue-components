...
## 边框圆角
使用 `rounded` 属性为边框添加圆角。此处使用`borderColor`配了个清晰的颜色。为了效果更直观这里使用了`paddingText`移动了文本的位置。
...
## 边框圆形
使用 `circular` 属性将边框变为圆形。此处使用`borderColor`配了个清晰的颜色。为了效果更直观这里使用了`paddingText`移动了文本的位置。
...
<br />
· 如果设置了`cricle`，`rounded`将不会生效，`cricle`和`rounded`只能同时使用一种样式。
<br />
